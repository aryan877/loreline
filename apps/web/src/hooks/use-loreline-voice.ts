    ...
    const prepareReaderResponse = tool({
      ...
      execute: async ({ mode, page, text }) => {
        ...
        if (mode === "inspect_pointer") {
          ...
          const inspection = await inspectRenderedPage("pointer");
          const pointerText = current.pointer?.text?.trim();
          if (!pointerText) return inspection;
          const located = await controlsRef.current.focusPassage({
            page: current.page,
            text: pointerText,
          });
          return located
            ? `${inspection} The exact sentence under the cursor is now persistently focused on the page: “${pointerText}”.`
            : `${inspection} The cursor is over “${pointerText}”, but that sentence could not be focused.`;
        }
        if (mode === "inspect_page") {
          controlsRef.current.clearFocus();
          return inspectRenderedPage("page");
        }
